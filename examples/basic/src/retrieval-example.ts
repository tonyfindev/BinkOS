...
import { RetrievalPlugin } from "@binkai/retrieval-plugin";
import { BinkProvider } from "@binkai/bink-provider";

async function main() {
  // Initialize plugin
  const retrievalPlugin = new RetrievalPlugin();
  ...
  await retrievalPlugin.initialize({
    providers: [binkProvider],
  });

  // Register with agent
  await agent.registerPlugin(retrievalPlugin);
